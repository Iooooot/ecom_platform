...
- 购物车
  ...
  添加商品到购物车，支持批量修改数量或删除；显示商品总价、优惠抵扣、预估运费；库存不足或下架商品自动标记，提示用户清理。
...
- 订单管理（用户端）
  ...
  查看订单状态（如待支付、已支付、已发货、已完成等）；取消未支付的订单（超时取消）；对已完成订单的商品进行评分和图文评价。
...
- 管理员商品管理
  ...
  - **商品管理** 
    ...
    - 创建满减、秒杀、优惠券活动，绑定指定商品。

- √用户管理（敏感操作部分），需要创建user_operation_logs
